...
// 获取 Electron dialog 模块
const electron = (window as any).require('electron');
const dialog = electron.remote ? electron.remote.dialog : electron.dialog;
const fs = electron.remote ? electron.remote.fs : electron.fs;
...
export class CreateTemplateModal extends Modal {
    ...
    private showJsonImport = false;
    private jsonTextArea: HTMLTextAreaElement;
    private jsonImportSection: HTMLElement;
  ...
  "styles": {
    ...
    private importJsonStyles() {
        ...
        try {
            ...
            // 支持两种格式：
            // 1. 完整的模板对象 { id, name, styles }
            // 2. 只有 styles 对象 { styles: {...} }
            // 3. 直接是 styles 对象 { paragraph: ..., title: ... }
            ...
            if (!this.validateStyles(importedStyles)) {
                new Notice('JSON 格式不正确，缺少必要的样式字段');
                return;
            }
            ...
            new Notice('JSON 样式导入成功！可以点击预览查看效果，或继续调整');

            // 可选：导入后关闭 JSON 导入模式，显示常规编辑界面
            // this.showJsonImport = false;
            // this.jsonImportSection.style.display = 'none';

        } catch (error) {
            new Notice('JSON 解析失败：' + error.message);
            console.error('JSON 解析错误:', error);
        }
    }

    private validateStyles(styles: any): boolean {
        // 检查必要的样式字段
    ...
    private async selectAndImportCssFile() {
        try {
            // 打开文件选择对话框
            ...
            new Notice('CSS 文件已转换，请点击"解析并应用 JSON"按钮应用样式');

        } catch (error) {
            new Notice('CSS 文件导入失败：' + error.message);
            console.error('CSS 导入错误:', error);
        }
    }

    private convertCssToTemplateStyles(css: string): any {
        // 默认样式结构
        const styles = {
            container: "",
            title: {
